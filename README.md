...
이산 환경과 연속 환경 모두에서 전체 작물 작업을 안정적으로 완료하는 정책을 학습했습니다. Step 3에서는 거리 관측과 DQN을 활용하여 가까운 미완료 레인을 선택하는 정책을 학습했으며, Step 4에서는 내비게이션 플래그와 단순화된 관측을 활용한 연속 제어에서도 높은 성능을 확인했습니다. 앞으로 인접한 작물 열을 순서대로 처리하는 주행 정책을 학습하려면 보상 구조, 하이퍼파라미터 및 모델 구성에 대한 추가 실험이 필요합니다.
