...
둘째, **부분 관측(POMDP)**이다. 로봇은 예찰(Scout) 행동을 하기 전에는 작물의 실제 상태(수확 대기/방제 대기/정상)를 알 수 없다. 즉 "정보를 얻는 행동(예찰)"과 "정보를 활용하는 행동(조치)"이 분리되어 있어, 에이전트(또는 계층적 구조에서는 상위 정책)는 탐색(exploration)과 활용(exploitation)의 순서를 스스로 조율해야 한다. 이 구조는 명시적 규칙으로 설계하기 까다로운 반면, 보상을 통한 학습으로는 자연스럽게 유도할 수 있다.

셋째, **정답 경로가 명시되지 않는다**. 어느 레인을 어떤 순서로 도는 것이 최적인지에 대한 지도학습용 라벨이 존재하지 않는다. 환경과의 상호작용을 통해 보상 신호만으로 정책을 발견해야 하므로 강화학습이 적합한 접근이다.
