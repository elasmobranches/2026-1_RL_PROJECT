...
|------|-------------------|-----------------------|------|
| **성공률** | 96.0% | **96.0%** | = 유지 |
| **평균 커버리지** | 98.8% | **99.5%** | +0.7% 향상 |
| **평균 완료 스텝** | 146.5 ± 80.4 | **157.2 ± 58.1** | +7% (안정성↑) |
| 레인 방문 횟수 | — | **평균 3.1회** | 5레인 중 3.1회만 방문 |

**주요 발견: 암묵적 다중 레인 커버리지**

High-level이 목표 레인을 지정하더라도, Low-level 에이전트는 목표 레인까지 이동하는 경로 중 **다른 레인의 인접 작물도 자연스럽게 예찰**한다. 이로 인해 평균 3.1회 레인 방문만으로 5개 레인 전체를 99.5% 커버리지로 처리할 수 있었다. 이는 계층 구조가 명시적으로 설계하지 않은 부수적 효율을 발휘한 것이다.

**스텝 수 분석**

Step 2의 평균 스텝(157)이 Step 1(147)보다 약간 많은 이유: High-level이 "목표 레인으로 이동"을 명시적으로 지시하므로 Low-level이 헤드랜드를 통한 명시적 이동을 수행한다. 반면 Flat RL(Step 1)은 이미 인접 작물을 처리하면서 자연스럽게 이동하는 패턴을 학습했다.

그러나 **스텝 분산이 80.4 → 58.1로 27% 감소**했으며, 이는 계층 구조가 더 일관되고 예측 가능한 동작을 보임을 의미한다.
...
| 성공률 ≥ 96% | ≥ 96% | 96.0% | ✅ |
| 평균 커버리지 ≥ 99% | ≥ 99% | 99.5% | ✅ |
| 평균 완료 스텝 ≤ 110 | ≤ 110 | 157.2 | ❌ (미달) |

스텝 감소 목표는 달성하지 못했으나, 성공률·커버리지 목표는 달성하였으며 동작 안정성(분산 감소)이 향상되었다.
...
본 프로젝트는 정밀농업 자율 로봇을 위한 커스텀 강화학습 환경을 설계하고 두 단계에 걸쳐 에이전트를 학습시켰다.

- **Step 1**: 2열 재배단 Gymnasium 환경 + MaskablePPO → **96% 성공률, 98.8% 커버리지**
- **Step 2**: 계층적 RL (LaneExecutorEnv + HighLevelFarmEnv) → **96% 성공률, 99.5% 커버리지, 동작 분산 27% 감소**

### 8.2 향후 연구

- **다중 로봇 협력 커버리지 (MARL)**: MAPPO 기반으로 복수 에이전트가 분할 예찰
- **Low-level 경로 효율화**: A* 또는 Boustrophedon 사전 경로와 RL 결합
- **실제 ROS2 배포**: 현재 Gymnasium 환경을 실제 ROS2 노드로 분리 및 로봇 연결
