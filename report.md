...
**Goal-reaching 효과**: 목표 레인 도달 보너스(+2.0)가 Low-level의 직접 이동을 일부 유인하나, Step 3가 Step 2 대비 열등한 이유는 DQN High-level의 학습 부족 때문으로 분석된다. DQN은 300k 스텝으로는 최적 레인 순서를 충분히 학습하지 못했으며(평균 5.6 방문), 레인 재방문이 잦아 전체 스텝이 증가하였다.

**Step 패널티 실험 (음성 결과)**: Low-level의 스텝 패널티를 -0.1 → -0.3으로 강화한 실험에서 성공률이 96% → 80%로 급락하였다. 강한 시간 압박이 "목표 레인 직행"을 유도하지 못하고 오히려 레인 완료 실패율을 높임을 확인하였다. 이는 단순한 보상 스케일링이 계층적 행동 개선에 효과적이지 않음을 시사한다.

**Step 2 우월성**: 계층적 PPO(Step 2)가 세 단계 중 가장 높은 성공률(98%)과 최소 레인 방문(2.3)을 달성하였다. Low-level이 헤드랜드 이동 중 주변 레인의 작물을 자동 예찰하는 부수적 효과가 전체 효율을 향상시켰다.
...
| 단계 | 핵심 기여 | 성공률 | 커버리지 |
|------|-----------|--------|---------|
| Step 1 | 2열 재배단 커스텀 env + MaskablePPO | 86~96% | 98~99% |
| Step 2 | 계층적 RL (PPO High-level) | **98%** | **99.8%** |
| Step 3 | Goal-reaching + DQN (탐색적 개선) | 78% | 94% |

계층적 RL(Step 2)이 Flat RL(Step 1)보다 성공률과 커버리지에서 우위를 보였으며, 레인 방문 최소화(2.3회)를 통해 효율적인 필드 커버리지를 달성하였다. Step 3의 DQN 적용은 탐색적 시도로서 추가 학습 시간이 주어지면 개선될 여지가 있다.

### 9.2 향후 연구

- **DQN 충분한 학습**: 300k → 1M 스텝으로 확장하여 Step 3 성능 개선 시도
- **SAC-Discrete 구현**: 최대 엔트로피 기반 이산 RL로 더 강건한 탐색
- **다중 로봇 협력 (MARL)**: MAPPO 기반 복수 에이전트 분할 예찰
