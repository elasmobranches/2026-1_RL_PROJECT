...
연속 환경의 희소 보상을 완화하기 위해 가장 가까운 미처리 작물까지의 거리를 이용한 잠재 기반 보상 형태화를 적용하였다. 환경 클래스는 스폰 난이도 조절 기능을 지원하지만, 최종 학습에서는 `curriculum_level=2`로 고정하여 모든 에피소드가 헤드랜드에서 시작하도록 설정하였다. 따라서 최종 SAC 성능은 커리큘럼 학습의 효과를 보여주는 결과가 아니며, 커리큘럼 적용 여부에 따른 성능 차이도 별도로 검증하지 않았다.
