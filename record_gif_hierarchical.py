...
TARGET_TINT  = [0.55, 0.85, 1.00]   # light blue overlay for active target lane
...
def render_frame(inner_env, step, coverage, lane_visit, target_col):
    ...
    # Highlight target lane (path cells only)
    for r in range(H):
    ...
    ax.set_title(
        f'Step {step:3d}  |  Coverage {coverage:.0%}  |  Lane Visit #{lane_visit}  |  Target Col {target_col}',
        fontsize=9, pad=6,
    )
...
def record(seed=3, out='agent_demo_hierarchical.gif', fps=5):
    ll_model = MaskablePPO.load('models/lane_executor')
    hl_model = PPO.load('models/high_level')
...
if __name__ == '__main__':
    record(seed=3, out='agent_demo_hierarchical.gif', fps=5)
